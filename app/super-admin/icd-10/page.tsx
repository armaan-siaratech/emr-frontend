...
export default function ICD10Page() {
  ...
  const [selectedItem, setSelectedItem] = useState<ICD10Item | null>(null);
  ...
  }, [search, selectedCategory, selectedStatus, showDeleted, page, pageSize]);
  ...
  const handleFileUpload = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!uploadFile) {
      setFormError("Please select a valid .xlsx or .csv file to upload.");
      return;
    }
    setFormError(null);
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div>
            <div className="mb-2 flex items-center gap-2 text-teal-200 text-xs font-bold uppercase tracking-wider">
              ...
              <span>/</span>
              <span className="text-white">ICD-10 Master Catalog</span>
            </div>

            <h1 className="text-2xl sm:text-3xl md:text-4xl font-black tracking-tight text-white flex items-center gap-3">
              <Sparkles className="w-7 h-7 text-teal-300 animate-pulse" />
              ICD-10 Diagnostic Catalog
            </h1>
            <p className="mt-2 text-xs sm:text-sm text-teal-100/90 max-w-2xl leading-relaxed">
              medical diagnosis catalog powered by live backend services. Manage active codes, soft-delete, 1-click restore, and bulk file uploads.
            </p>
          </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 rounded-3xl border-2 border-[#7ee8d5]/60 bg-white/95 p-4 shadow-[0_10px_30px_rgba(15,118,110,0.06)] backdrop-blur-2xl">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
            <label className="flex items-center gap-1.5 text-xs text-[#596964] font-semibold cursor-pointer">
              <input
                type="checkbox"
                checked={showDeleted}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {items.map((item) => (
              <motion.div
                ...
                className={`relative group rounded-3xl border-2 p-5 shadow-[0_10px_25px_rgba(15,118,110,0.08)] hover:shadow-[0_20px_45px_rgba(15,118,110,0.22)] hover:-translate-y-1.5 transition-all duration-300 backdrop-blur-xl flex flex-col justify-between overflow-hidden ${item.is_deleted
              ...
              >
                ...
                <div className="space-y-3 relative z-10">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="bg-gradient-to-r from-[#0F766E] to-[#115e59] text-white shadow-md shadow-teal-900/25 font-mono font-black text-sm px-3.5 py-1.5 rounded-2xl flex items-center gap-1.5 border border-teal-400/40">
                      <Hash className="w-3.5 h-3.5 text-teal-200" />
                      {item.code}
                    </span>

                    <div>
                      {item.is_deleted ? (
                        <span className="inline-flex items-center gap-1 rounded-full bg-rose-100 px-2.5 py-1 text-[10px] font-bold text-rose-700 border border-rose-200">
                  ...
                  {/* Description */}
                  <div>
                    <h3 className={`text-xs font-bold leading-relaxed font-sans ${item.is_deleted ? "line-through text-slate-400" : "text-[#132a26]"}`}>
                      {item.description}
                    </h3>
                  </div>

                  {/* Category & Chapter Tag Pills */}
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    <span className="rounded-xl bg-[#E7F4F1] border border-[#bce4db] px-2.5 py-1 text-[10px] font-bold text-[#0F766E]">
                      {item.category || "General Diagnosis"}
                    </span>

                    {item.parent_code && (
                      <span className="rounded-xl bg-slate-100 border border-slate-200 px-2.5 py-1 text-[10px] font-mono font-bold text-slate-600">
                        Parent: {item.parent_code}
                      </span>
                    )}

                    <span className="rounded-xl bg-amber-50 border border-amber-200 px-2 py-1 text-[10px] font-mono font-bold text-amber-700">
                      v{item.version || "2026"}
                    </span>
                  </div>

                  {item.chapter && (
                    <p className="text-[10px] text-[#63827a] font-medium line-clamp-1 border-t border-teal-100/80 pt-2">
                      {item.chapter}
                    </p>
                  )}
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
            ...
            >
              ...
              <form onSubmit={handleFileUpload} className="space-y-5 relative z-10">
                <div className="rounded-2xl border border-white/80 bg-white/45 p-6 shadow-sm backdrop-blur-md text-center border-2 border-dashed border-[#7ee8d5]/80 hover:border-[#0f766e] transition cursor-pointer relative">
                  <input
                    type="file"
                    accept=".xlsx,.xls,.csv"
                    onChange={(e) => {
                  ...
                  {uploadFile ? (
                    <div>
                      <p className="font-black text-sm text-[#132a26]">{uploadFile.name}</p>
                      <p className="text-xs text-[#0f766e] font-bold mt-1">
                        {(uploadFile.size / 1024).toFixed(1)} KB • Ready to import into database
                      </p>
                      <p className="text-[10px] text-teal-700 font-medium mt-1">
                        High-Speed Batch Engine Enabled: Optimized for 50,000+ records without UI freezing.
                      </p>
                    </div>
                  ) : (
                    <div>
